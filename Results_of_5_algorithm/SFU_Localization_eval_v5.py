...
'''def se(x, locations, distances):
    se = 0.0
    cnt = 0
    for location, distance in zip(locations, distances):
        if distance != 0:
            distance_calculated = cricle_distance(x[0], x[1], location[0], location[1])
            se += float(math.pow(distance_calculated - distance, 2.0)) / float((VAR_dist(distance)))
            cnt += 1
    if cnt == 0:
        return float('nan')
    else:
        return se'''
def se(params, locations, distances, data_esp):
    x=[params['x'].value,params['y'].value]
    se = []
    cnt=0
    for location, distance,esp in zip(locations, distances,data_esp):
        if distance !=0 and esp!=0:
            distance_calculated = cricle_distance(x[0], x[1], location[0], location[1])
            se.append(float(math.pow(distance_calculated - distance, 2.0)) /float((VAR_dist(distance))))
            cnt+=1
...
while True:
    ...
    # print (packet)
    item = packet.split(",")
    if item[1] != 'f1bbccddeeaa'  and item[1]!='f2bbccddeeaa': # and item[1] != 'f2bbccddeeaa' no. of fixed scanners evaluation
        continue
    ...
    if any(vt >= MAX_NUM for vt in temp_ble):
        ...
        print "est_distances"
        print d
        #init_est_pos[0]=FS_posX[max_i] #+= force_rss[ind] * FS_posX[ind]
        #init_est_pos[1]= FS_posY[max_i]#+= force_rss[ind] * FS_posY[ind]

        #print avg
            # initial_location: (lat, long)
        # locations: [ (lat1, long1), ... ]
        # distances: [ distance1,     ... ]
        #Initial confidence Region


        if fs_cnt > 2:
            if buffX1:
                if new_time - buff_time[-1] < wndw:
                    init_est_pos[0] = buffX1[-1]
                    init_est_pos[1] = buffY1[-1]
                else:
                    ...
                    # print ref_id
                    init_est_pos = [Ref_posX[ref_id - 1], Ref_posY[ref_id - 1]]
                    '''init_est_pos[0] = FS_posX[max_i]  # force_rss[ind] * FS_posX[ind]
                    init_est_pos[1] = FS_posY[max_i]  # force_rss[ind] * FS_posY[ind]'''
            else:
                stk = FS_reg_dist[max_i][:]
            ...
            # initial esitimate
            params = Parameters()
            params.add('x',value=init_est_pos[0],vary=True,min=-1000,max=None)
            params.add('y',value=init_est_pos[1],vary=True,min=None,max=1000)
            # print inti_est_pos
            '''result = optimize.minimize(
                se,  # The error function
                init_est_pos,  # The initial guess
                args=(FS_pos, d),  # Additional parameters for mse
                bounds=[(0, None), (None, 0)],
                method='L-BFGS-B',  # The optimisation algorithm
                options={
                    'ftol': 1e-20,  # Tolerance
                    'maxiter': 1e+20  # Maximum iterations
                })'''
            mini=[]
            result=[]
            try:
                mini = lmfit.Minimizer(
                    se,  # The error function
                    params,  # The initial guess
                    fcn_args=(FS_pos, d,d_ci)  # Additional parameters for mse
                    )
                result = mini.minimize(ftol=1e-20)
                ...
                # estimating confidence region
                ci = conf_interval(mini, result, sigmas=[0.95])
                b_pos = [ci['x'][1][1], ci['y'][1][1]]
                CI = [[], []]
                CI[0] = [ci['x'][1][1] - ci['x'][0][1], ci['x'][2][1] - ci['x'][1][1]]
                CI[1] = [ci['y'][1][1] - ci['y'][0][1], ci['y'][2][1] - ci['y'][1][1]]
            except:
                pass
            #lmfit.printfuncs.report_ci(ci)

            for ici in [0,1]:
                if CI[ici][0] == float('Inf') or CI[ici][0] == -float('Inf'):
                    CI[ici][0]=CI[ici][1]
                    continue
                if CI[ici][1] == float('Inf') or CI[ici][1] == -float('Inf'):
                    CI[ici][1]=CI[ici][0]
                    continue
                CI[ici]=[sum(CI[ici])/2,sum(CI[ici])/2]

            if buffX1:
                # print 'test1'
                # print new_time - buff_time[-1]
                if new_time - buff_time[-1] < wndw:
                    #print 'test3'
                    print cricle_distance(b_pos[0], b_pos[1], buffX1[-1], buffY1[-1])
                    if cricle_distance(b_pos[0], b_pos[1], buffX1[-1], buffY1[-1]) > 2:
                        #print "test2"
                        b_pos[0] = buffX1[-1]
                        b_pos[1] = buffY1[-1]
            ...
            if fs_cnt==1:
                stk = FS_Ref_dist[max_i][:]
                idf=int(find_nearest(stk, d[max_i]))
                b_pos=[Ref_posX[idf],Ref_posY[idf]]
                CI=[[d[max_i]-d_ci[max_i],d[max_i]+d_ci[max_i]],[d[max_i],d[max_i]]]
                ...
                # print ref_id
                b_pos = [(Ref_posX[ref_id1 - 1]+Ref_posX[ref_id2 - 1]+Ref_posX[ref_id3 - 1])/3, (Ref_posY[ref_id1 - 1]+Ref_posY[ref_id2 - 1]+Ref_posY[ref_id3 - 1])/3] # estimated beacon position
                #CI = [[d_ci[max_i],d_ci[max_i]], [d_ci[max_i],d_ci[max_i]]]
                CI=[[d[max_i]-d_ci[max_i],d[max_i]+d_ci[max_i]],[d[max_i],d_ci[max_i]]]
